...
![alt text](https://img.shields.io/badge/License-GPLv3-green.svg)
...
📖 项目简介
Cai Install Web GUI 是原版 Cai-Install 的重构版本，外观设计灵感来源于 Onekey_GUI。本项目采用 Flask + Vue/原生JS 架构构建，抛弃了传统的终端界面，为您提供了一个拥有 Material Design 3 风格的精美图形化界面。
...
依赖软件：SteamTools 或 GreenLuma (需提前安装其中之一)
开发环境 (仅针对从源码运行)：Python 3.8 或更高版本。
方式一：下载整合包 (推荐)
前往 Releases 页面，下载最新的打包版本，解压后双击运行 .exe 文件即可启动。
方式二：从源码运行
克隆本仓库到本地：
code
Bash
